...
# Prefer env var, fallback to Streamlit secrets, and allow user entry
...
with st.sidebar:
    ...
    if not GROQ_API_KEY:
        GROQ_API_KEY = st.text_input(
            "Groq API Key",
            type="password",
            help="Set GROQ_API_KEY here if .env is not configured",
        )
    st.caption("Model: mixtral-8x7b-32768 · Temperature: 0.0")

if not GROQ_API_KEY:
    st.error("GROQ_API_KEY is not set. Add it to .env or enter it in the sidebar.")
    st.stop()
